import React from "react";
import BrandLogo from "./BrandLogo";
import { runtimeConfig } from "../config/runtime";

const FOOTER_LINKS = [
  {
    heading: "Formation",
    links: [
      { label: "S'orienter", href: "https://www.unistra.fr/fr/formation/sorienter" },
      { label: "Toutes les formations", href: "https://formations.unistra.fr/" },
      { label: "Licences", href: "https://formations.unistra.fr/fr/formations/licence-LIG.html" },
      { label: "Masters", href: "https://formations.unistra.fr/fr/formations/master-MAS.html" },
      { label: "Doctorat", href: "https://doctorat.unistra.fr/" },
      { label: "Formation continue", href: "https://www.unistra.fr/fr/formation/continuer-se-former" },
      { label: "Calendrier universitaire", href: "https://www.unistra.fr/fr/formation/admission/calendrier-universitaire" },
    ],
  },
  {
    heading: "Recherche",
    links: [
      { label: "Unités de recherche", href: "https://www.unistra.fr/fr/recherche/les-unites-de-recherche" },
      { label: "Écoles doctorales", href: "https://doctorat.unistra.fr/" },
      { label: "Transfert & valorisation", href: "https://www.unistra.fr/fr/recherche/transfert-de-technologies" },
      { label: "Savoir(s) — média", href: "https://savoirs.unistra.fr/" },
      { label: "Actualités recherche", href: "https://www.unistra.fr/fr/actualites/recherche" },
    ],
  },
  {
    heading: "Université",
    links: [
      { label: "Gouvernance", href: "https://www.unistra.fr/fr/universite/gouvernance" },
      { label: "Facultés, écoles, instituts", href: "https://www.unistra.fr/fr/entites" },
      { label: "Chiffres clés", href: "https://www.unistra.fr/fr/universite/decouvrir/chiffres-cles" },
      { label: "Recrutement", href: "https://www.unistra.fr/les-offres-demploi" },
      { label: "Presse & médias", href: "https://www.unistra.fr/fr/universite/presse-et-medias" },
      { label: "Plan du site", href: "https://www.unistra.fr/fr/sitemap" },
    ],
  },
  {
    heading: "Vie universitaire",
    links: [
      { label: "Vie étudiante", href: "https://www.unistra.fr/fr/vie-universitaire/vie-etudiante" },
      { label: "Sport & culture", href: "https://www.unistra.fr/fr/vie-universitaire/sport-culture" },
      { label: "Bibliothèques", href: "https://www.unistra.fr/fr/vie-universitaire/bibliotheques" },
      { label: "Santé & handicap", href: "https://www.unistra.fr/fr/vie-universitaire/sante-handicap" },
      { label: "Carte des campus", href: "https://www.unistra.fr/fr/carte" },
    ],
  },
];

const SOCIAL_LINKS = [
  {
    label: "Facebook",
    href: "https://www.facebook.com/unistra/",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 16 16" aria-hidden="true">
        <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z"/>
      </svg>
    ),
  },
  {
    label: "Instagram",
    href: "https://www.instagram.com/unistra/",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 16 16" aria-hidden="true">
        <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.917 3.917 0 0 0-1.417.923A3.927 3.927 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.916 3.916 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.926 3.926 0 0 0-.923-1.417A3.911 3.911 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0h.003zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599.28.28.453.546.598.92.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.47 2.47 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.478 2.478 0 0 1-.92-.598 2.48 2.48 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233 0-2.136.008-2.388.046-3.231.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92.28-.28.546-.453.92-.598.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045v.002zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92zm-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217zm0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334z"/>
      </svg>
    ),
  },
  {
    label: "YouTube",
    href: "https://www.youtube.com/@unistra",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 16 16" aria-hidden="true">
        <path d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104.022.26.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105-.009.104c-.05.572-.124 1.14-.235 1.558a2.007 2.007 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006-.087-.004-.171-.007-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.007 2.007 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31.4 31.4 0 0 1 0 7.68v-.123c.002-.215.01-.958.064-1.778l.007-.103.003-.052.008-.104.022-.26.01-.104c.048-.519.119-1.023.22-1.402a2.007 2.007 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007.172-.006.086-.003.171-.007A99.788 99.788 0 0 1 7.858 2h.193zM6.4 5.209v4.818l4.157-2.408L6.4 5.209z"/>
      </svg>
    ),
  },
  {
    label: "LinkedIn",
    href: "https://www.linkedin.com/school/université-de-strasbourg/",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 16 16" aria-hidden="true">
        <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854V1.146zm4.943 12.248V6.169H2.542v7.225h2.401zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248-.822 0-1.359.54-1.359 1.248 0 .694.521 1.248 1.327 1.248h.016zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016a5.54 5.54 0 0 1 .016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225h2.4z"/>
      </svg>
    ),
  },
];

const FooterStrasbourg = () => {
  const primary = runtimeConfig.primaryColor;

  return (
    <footer style={{ fontFamily: "Arial, Helvetica, sans-serif", marginTop: "40px" }}>
      {/* ── Main footer body ── */}
      <div style={{ backgroundColor: primary, color: "#fff", padding: "40px 0 28px" }}>
        <div
          style={{
            maxWidth: "1200px",
            margin: "0 auto",
            padding: "0 16px",
            display: "grid",
            gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))",
            gap: "32px 24px",
          }}
        >
          {/* Logo + tagline column */}
          <div>
            <a href="https://www.unistra.fr/" target="_blank" rel="noopener noreferrer">
              <BrandLogo
                alt={runtimeConfig.universityName}
                inverse
                style={{ height: "52px", width: "auto", marginBottom: "14px" }}
              />
            </a>
            <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.75)", lineHeight: "1.6", marginTop: "12px" }}>
              Université européenne de service public, engagée pour la réussite
              en formation et en recherche.
            </p>
            {/* Social links */}
            <div style={{ display: "flex", gap: "12px", marginTop: "16px", flexWrap: "wrap" }}>
              {SOCIAL_LINKS.map((s) => (
                <a
                  key={s.label}
                  href={s.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={s.label}
                  style={{
                    color: "rgba(255,255,255,0.8)",
                    transition: "color 0.15s",
                  }}
                  onMouseEnter={(e) => (e.currentTarget.style.color = "#fff")}
                  onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(255,255,255,0.8)")}
                >
                  {s.icon}
                </a>
              ))}
            </div>
          </div>

          {/* Navigation columns */}
          {FOOTER_LINKS.map((col) => (
            <div key={col.heading}>
              <h3
                style={{
                  fontSize: "13px",
                  fontWeight: "700",
                  textTransform: "uppercase",
                  letterSpacing: "0.8px",
                  marginBottom: "12px",
                  color: "#fff",
                  borderBottom: "1px solid rgba(255,255,255,0.25)",
                  paddingBottom: "8px",
                }}
              >
                {col.heading}
              </h3>
              <ul style={{ listStyle: "none", margin: 0, padding: 0 }}>
                {col.links.map((link) => (
                  <li key={link.label} style={{ marginBottom: "6px" }}>
                    <a
                      href={link.href}
                      target="_blank"
                      rel="noopener noreferrer"
                      style={{
                        color: "rgba(255,255,255,0.8)",
                        textDecoration: "none",
                        fontSize: "13px",
                        transition: "color 0.15s",
                      }}
                      onMouseEnter={(e) => (e.currentTarget.style.color = "#fff")}
                      onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(255,255,255,0.8)")}
                    >
                      {link.label}
                    </a>
                  </li>
                ))}
              </ul>
            </div>
          ))}
        </div>
      </div>

      {/* ── Bottom bar ── */}
      <div
        style={{
          backgroundColor: "#111a2e",
          color: "rgba(255,255,255,0.6)",
          fontSize: "12px",
          padding: "12px 16px",
        }}
      >
        <div
          style={{
            maxWidth: "1200px",
            margin: "0 auto",
            display: "flex",
            justifyContent: "space-between",
            flexWrap: "wrap",
            gap: "8px",
            alignItems: "center",
          }}
        >
          <span>© {new Date().getFullYear()} Université de Strasbourg — 4 rue Blaise Pascal, 67081 Strasbourg Cedex</span>
          <div style={{ display: "flex", gap: "16px", flexWrap: "wrap" }}>
            <a href="https://www.unistra.fr/fr/credits-et-mentions-legales" target="_blank" rel="noopener noreferrer" style={{ color: "rgba(255,255,255,0.6)", textDecoration: "none" }}>Mentions légales</a>
            <a href="https://www.unistra.fr/fr/protection-des-donnees" target="_blank" rel="noopener noreferrer" style={{ color: "rgba(255,255,255,0.6)", textDecoration: "none" }}>Protection des données</a>
            <a href="https://www.unistra.fr/fr/politique-daccessibilite" target="_blank" rel="noopener noreferrer" style={{ color: "rgba(255,255,255,0.6)", textDecoration: "none" }}>Accessibilité</a>
            <a href="https://www.unistra.fr/fr/contact" target="_blank" rel="noopener noreferrer" style={{ color: "rgba(255,255,255,0.6)", textDecoration: "none" }}>Contact</a>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default FooterStrasbourg;
